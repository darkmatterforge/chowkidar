...
test.describe('Settings — persist through container restart', () => {
  test('primaryBaseURL, externalHostname, displayTimezone and serverTimezone survive restart', async ({ page }) => {
    // Save known values for all four fields.
    ...
    await page.request.put(`${BASE_URL}/api/settings`, {
      data: {
        ...
        // preserve required numeric fields
        workerCount:       2,
        queueSize:         64,
    ...
    // Verify all four fields survived the restart
    ...
    // Restore
    await page.request.put(`${BASE_URL}/api/settings`, {
      data: { ...s, primaryBaseURL: '', externalHostname: '' },
    })
  })
})
